        ...
        <fieldset>
          ...
          <div v-for="(item, index) in siteConfig.contactInfo.items" :key="index" class="dynamic-item">
            ...
            <div class="form-group">
              ...
              <input :id="'contact-value-' + index" type="text" v-model="item.value" placeholder="例如：contact@example.com">
            </div>
            <div class="form-group">
              <label :for="'contact-url-' + index">项目 {{ index + 1 }}: 链接 (可选)</label>
              <input :id="'contact-url-' + index" type="text" v-model="item.url" placeholder="例如：mailto:contact@example.com">
            </div>
            <button type="button" @click="removeContactItem(index)" class="api-button small-button danger">移除此项</button>
          </div>
        ...
        <table class="admin-table">
          ...
          <tbody>
            <tr v-for="user in users" :key="user.id">
              ...
              <td>{{ user.email }}</td>
              <td>
                <select v-model="user.role" @change="updateUserRole(user, $event.target.value)" :disabled="user.id === authStore.user?.id">
              ...
              <td>
                <button @click="toggleUserStatus(user)" class="api-button small-button" :disabled="user.id === authStore.user?.id">切换激活</button>
                <button @click="deleteUser(user)" class="api-button small-button danger" :disabled="user.id === authStore.user?.id">删除</button>
              </td>
            </tr>
          </tbody>
        </table>
